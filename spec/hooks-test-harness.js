...
const useRef = (initial) => getInternalState(() => ({ current: initial }));
...
const useLayoutEffect = (fn, deps) => _useEffect(POST_RENDER_LAYOUT, fn, deps);
const useEffect = (fn, deps) => _useEffect(POST_RENDER_EFFECT, fn, deps);

function useReducer(reducer, initialArg, init = (x) => x) {
...
const useState = (initial) => useReducer(stateReducer, initial, stateInit);
